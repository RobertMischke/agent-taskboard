...
| Platform architecture index (durable execution and delivery architecture extracted from decision dossiers) | [concepts/platform-architecture/README.md](../concepts/platform-architecture/README.md) |
| Fencing, leases, and attempt authority (single-writer contract, lease TTLs, fence equality, authority epoch, replay and idempotency) | [concepts/platform-architecture/fencing-leases-and-authority.md](../concepts/platform-architecture/fencing-leases-and-authority.md) |
| Rebase, merge, and bounce steering (integration invariants, stage matrix, canonical integration ladder, failure classification, integration records) | [concepts/platform-architecture/rebase-merge-and-integration-invariants.md](../concepts/platform-architecture/rebase-merge-and-integration-invariants.md) |
| Task Server topology (process map, state ownership, local-v1 versus proxied plane, route surface, Git evidence model, AGT-2663 cutover state) | [concepts/platform-architecture/task-server-topology.md](../concepts/platform-architecture/task-server-topology.md) |
| Batch Gate mechanics (closed manifest, mechanical replay, single-candidate authorisation, bounded halving) | [concepts/platform-architecture/batch-gate.md](../concepts/platform-architecture/batch-gate.md) |
| Organization-wide telemetry layer (existing signal inventory, proposed event envelope, privacy boundary, transport, signal-to-action vocabulary) | [concepts/platform-architecture/telemetry-layer.md](../concepts/platform-architecture/telemetry-layer.md) |
| Remote Gate target architecture (GateSubject/GateAttempt/GateLease object model, capability-based claims, exact-SHA materialization, timeout-versus-infra-retry taxonomy, AGT-2262 cutover gate) | [concepts/platform-architecture/remote-gate.md](../concepts/platform-architecture/remote-gate.md) |
...
| Remote gate target architecture decision dossier (claimable gate steps vs Review Plane vs capability-only dispatch; AGT-2369) | [decision dossier](../operations/remote-gate-zielbild/index.html) · [architecture page](../concepts/platform-architecture/remote-gate.md) |
